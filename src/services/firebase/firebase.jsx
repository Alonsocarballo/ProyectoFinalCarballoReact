...
// Obtiene una referencia a la colección "products" en Firestore
...
// Función para guardar los productos en Firebase Firestore
const saveProductsToFirebase = async () => {
  try {
    for (const products of products) {
      // Guarda cada producto en la colección "products" de Firestore
      await db.doc(Products.id).set(Products);
    }
    console.log('Productos guardados en Firebase Firestore correctamente.');
  } catch (error) {
    console.error('Error al guardar los productos en Firebase Firestore:', error);
  }
};

// Llama a la función para enviar los productos a Firebase Firestore
